...
class DeepSetsEntireNetwork:
    def __init__(self, input_dim, output_dim, task, after_network=False, last_linear=False, connect_dim=64, normalize=False):
        self.after_network = after_network
        self.normalize = normalize
        if after_network:
            self.deepsets = DeepSets(input_dim, connect_dim)
            ...
            self.deepsets = DeepSets(input_dim, output_dim)
            if "cls" in task:
        ...
        self.ds_params = self.deepsets.ds_weight1 + self.deepsets.ds_bias1 + self.deepsets.ds_weight2 + self.deepsets.ds_bias2
        
    def output(self, X, rot=False):
        if rot:
            X = random_rotation(X)
        ds_output = self.deepsets.output(X)
        if self.normalize:
            ds_output = nn.functional.normalize(ds_output, dim=0)
        if self.after_network:
            return self.after_mlp.output(ds_output)
        else:
            return self.after_func(ds_output)
...
def learning_process(args_dict, setting_suffix, CV_idx=None, fold_num=10):
    ...
    if optimizer == "sgd":
        opt = torch.optim.SGD(model.ds_params + model.other_params, lr=lr)
    elif optimizer == "adam":
        opt = torch.optim.Adam(model.ds_params + model.other_params, lr=lr)
    else:
        raise NotImplementedError
    iter_max = iter_num
...
if __name__ == "__main__":
    ...
    if (args.exp_id is not None) and "fold" in args.exp_id:
        ...
        for i in range(fold_num):
            _setting_suffix = f"DeepSets-{i}of{fold_num}{setting_suffix}"
        ...
        _setting_suffix = f"DeepSets_{args.exp_id},{setting_suffix}"
        learning_process(args_dict, setting_suffix=_setting_suffix)
    else:
        _setting_suffix = f"DeepSets,{setting_suffix}"
        learning_process(args_dict, setting_suffix=_setting_suffix)